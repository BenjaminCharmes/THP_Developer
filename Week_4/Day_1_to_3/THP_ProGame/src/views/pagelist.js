...
function PageList(argument) {
  const preparePage = () => {
          ...
          .then((data) => {
            ...
            publishersBtn.forEach((element, i) => {
              element.addEventListener('click', () => {
                pagelistPublisher(data.publishers[i].id)
              })
            });
    ...
    fetchList(`https://api.rawg.io/api/games?dates=${today},${oneYearLater}&ordering=-rating&page_size=27&key=${RAWG_KEY}`, cleanedArgument);
  };
...
form.addEventListener('submit', (event) => {
  ...
  PageList(searchValue);
});

const pagelistPublisher = async (argument) => {
